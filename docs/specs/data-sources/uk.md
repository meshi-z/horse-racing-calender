...
本ドキュメントは、イギリス競馬（British Horseracing Authority: BHA統轄、IFHA Part I平地全156重賞競走、および主要障害重賞全34競走: チェルトナムフェスティバル全G1、グランドナショナル、キングジョージ6世チェイス等計190競走）に関するデータ仕様書です。
...
| **レースID採番ルール (`id`)** | `{YYYY}-uk-{grade}-{index}` | 例: `2026-uk-g1-01`, `2026-uk-g1-54` (チェルトナムGC) |
| **格付け体系 (`grades`)** | `G1`, `G2`, `G3` | IFHA Part I 平地重賞（全156競走）およびBHA Jump Pattern 障害重賞（計34競走） |
| **マスタファイルパス** | `src/data/uk_race_master.json` | 日英辞書、全18競馬場マスタ、推定発走時刻 |
| **生成・ビルドモジュール** | `scripts/lib/uk-races.ts`, `scripts/parse-races.ts` | IFHA Part I・BHA Jump PatternとBHA公式カレンダーを統合 |
...
| **格付け・出走条件 (平地)** | IFHA Part I Great Britain リスト | PDF | レース格付け、出走資格、斤量、距離、馬場 |
| **格付け・出走条件 (障害)** | BHA Jump Pattern Committee リスト | Web / PDF | 障害レース格付け、出走資格、斤量、距離、障害種別 |
...
- **標準推定発走時刻（マスタ初期値）**:
  - 平地主要競走（エプソムダービー、キングジョージ等）: 現地 15:30〜16:35（夏時間: UTC `14:30`〜`15:35` / JST `23:30`〜`00:35`）
  - チェルトナムフェスティバル: 現地 13:30〜17:30（GMT: UTC `13:30`〜`17:30` / JST `22:30`〜`02:30`）
  - グランドナショナル: 現地 16:00（BST: UTC `15:00` / JST `00:00`）
  - 通常重賞競走: 現地 13:30〜16:00 前後
...
| :--- | :--- | :--- |
| **アスコット** | Ascot | ロイヤルアスコット、キングジョージ6世&QES、クラレンスハウスチェイス、アスコットチェイス |
| **ニューマーケット** | Newmarket | 2000ギニー、1000ギニー (ローリーマイル/ジュライ) |
| **エプソム** | Epsom | ダービーステークス、オークスステークス |
| **チェルトナム** | Cheltenham | チェルトナムフェスティバル全G1（ゴールドC、チャンピオンハードル等） |
| **エイントリー** | Aintree | グランドナショナルフェスティバル（グランドナショナル、エイントリーボウル等） |
| **ケンプトン** | Kempton | キングジョージ6世チェイス、クリスマスハードル、AW平地重賞 |
| **ヨーク** | York | インターナショナルステークス (イボア開催) |
| **グッドウッド** | Goodwood | サセックスステークス (グロリアスグッドウッド) |
| **ドンカスター** | Doncaster | セントレジャーステークス (最古のクラシック) |
| **サンダウン** | Sandown | エクリプスステークス、ティングルクリークチェイス、セレブレーションチェイス |
| **ニューベリー** | Newbury | ロッキンジステークス |
| **ヘイドック** | Haydock | スプリントカップ、ベットフェアチェイス |
| **ニューカッスル** | Newcastle | ファイティングフィフスハードル、AW直線コース |
...
- **採用馬場種別**:
  ...
  - `obstacle` (障害): ナショナルハント競走（チェイス / スティープルチェイス、ハードル、バンパー）
    - **チェルトナムフェスティバル (3月中旬)**: 全14G1競走（チェルトナムゴールドC、チャンピオンハードル、クイーンマザーチャンピオンチェイス、ステイヤーズハードル等）
    - **エイントリーフェスティバル (4月上旬)**: 全9G1競走（エイントリーボウル、エイントリーハードル等）および世界最高峰障害競走**グランドナショナル**（Premier Handicap / `grade: G3`）
    - **ボクシングデー・フェスティバル (12月26日)**: ケンプトン競馬場の伝統G1（キングジョージ6世チェイス、クリスマスハードル等）
