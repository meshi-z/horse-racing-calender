...
- **海外競馬：香港競馬（HKJC / IFHA Part I 重賞）統合 (v1.30.0 / Issue #109, #110, #111, #112)**:
  - **一次ソースからのデータ構築**: IFHA Part I リスト (Hong Kong: 格付け・出走資格・距離・馬場) および Hong Kong Jockey Club (HKJC) 公式開催日程に基づく平地全31重賞（G1 12競走、G2 7競走、G3 12競走）および4歳クラシックシリーズ（香港クラシックマイル、香港クラシックカップ、香港ダービー）の構造化。香港国際競走（香港カップ、香港マイル、香港スプリント、香港ヴァーズ）、チャンピオンズデー（QE2世C、チャンピオンズマイル、チェアマンズスプリントプライズ）、香港三冠（スチュワーズC、香港ゴールドC、チャンピオンズ&チャターC）等を含む。
  - **データアーキテクチャ拡張**:
  ...
  - **日英中対応マスタ (`src/data/hk_race_master.json`)**:
    ...
    - 抽出・マージモジュール（`scripts/lib/hk-races.ts`）の実装とビルドパイプライン（`scripts/parse-races.ts`）への統合。
  - **UI/UX 拡張**:
