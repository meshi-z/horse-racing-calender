...
| **バージョン** | v1.33.2 (言語切替ボタンタップ時にヘッダーおよび検索・フィルターエリアが消失する不具合の修正) |
...
### 現在地: Step 40 (複数国選択時のラベル長超過によるリセットボタンはみ出し・横揺れ・sticky固定解除および言語切替時ヘッダー消失の解消) [完了]
...
- スクロールロックと干渉していた `overflow-x: clip` を撤去し、コンテナの `max-w-full` 幅制御とすることで、言語切替ドロップダウン展開時のヘッダー・FilterBar消失を解消。
- 単体テストの拡充（複数国選択時の表示確認、言語切替ドロップダウン展開時のヘッダー維持等）および全462件テスト通過確認。
