...
- データの出典および免責事項（Disclaimer）の明記（非公式サイト表示、公式データ加工明記、免責ダイアログの実装） [完了]
...
### 4.6 免責事項 & データ出典モーダル (`DisclaimerDialog`)

- **非公式性・出典の明示:** フッターに常時非公式注記を表示するとともに、「免責事項・データ出典」を開くアクセシブルなトリガーを配置。
- **モーダル構成:** Shadcn UI の `Dialog` コンポーネントを使用し、以下4点を構造化して明記。
  1. **非公式ファンサイト:** 個人運営のファンサイトであり、JRAおよび競馬主催団体とは一切無関係である旨。
  2. **データの出典:** JRA公式サイト等で一般公開されているデータ（カレンダー、出馬表等）を取得・加工して提供している旨。
  3. **開催変更と免責規定:** 天候等による変更・中止の可能性、最新公式発表確認の推奨、利用に伴う損害への免責規定。
  4. **権利・商標の帰属:** レース名・競馬場名等の知的財産権が各権利者に帰属する旨。
- **アクセシビリティ & レスポンシブ対応:** 長文閲覧時の縦スクロール対応（`max-h-[85vh] overflow-y-auto`）、キーボード操作（Tab, Enter, Space, Esc）およびフォーカス管理を担保。
...
12. **Step 12:** アプリ内にデータの出典および免責事項（Disclaimer）を明記（`DisclaimerDialog`、フッター導線、各種ドキュメント更新、`v1.5.1` リリース）。[完了]
13. **Step 13 (Next):** フェーズ2 拡張機能（NAR/海外競馬対応、多言語UI切替、詳細フィルター等）の順次実装。[次のステップ]