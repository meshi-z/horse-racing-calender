...
| **バージョン** | v1.21.0 (フランス競馬発走予定時刻自動更新パイプライン整備および全過去重賞確定バックフィルの完了) |
...
v1.16.0 の多言語（日/英）対応、v1.17.0 のNAR全重賞・ばんえい競馬対応、v1.18.0 のアコーディオンフィルター、v1.18.1 のNAR英語名整理、v1.18.2 のヘッダー英語・検索例・SEO再整理、v1.19.0 のフランス平地重賞統合、v1.20.0 のNAR確定発走予定時刻自動更新パイプライン拡充、v1.20.1 のNAR過去重賞バックフィル（199レース）、v1.20.2 のJRA過去重賞バックフィル（98レース）を経て、**v1.21.0 ではフランス競馬（France-Galop）について、PMU公式APIを活用した高精度な確定発走予定時刻自動更新パイプライン（FranceRaceTimeFetcher）を実装。さらに2026年公式日程再編に伴う廃止競走（Prix Pénélope）の整理と、本日（2026年9月20日）以前に開催されたフランス重賞88レースの実績発走時刻バックフィルを完了。これにより、国内外（JRA・NAR・France）を合わせた過去開催重賞全386レースすべてが確定発走時刻（is_time_confirmed: true）となった**。
