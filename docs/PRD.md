...
| **作成日** | 2026年9月12日 (最終更新: 2026年9月24日) |
...
- **海外競馬：香港競馬（HKJC / IFHA Part I 重賞）統合 (v1.30.0 / Issue #109, #110, #111, #112)**:
  - **一次ソースからのデータ構築 [完了]**: IFHA Part I リスト (Hong Kong: 格付け・出走資格・距離・馬場) および Hong Kong Jockey Club (HKJC) 公式開催日程に基づく平地全31重賞（G1 12競走、G2 7競走、G3 12競走）および4歳クラシックシリーズ（香港クラシックマイル、香港クラシックカップ、香港ダービー）全35競走の構造化。香港国際競走（香港カップ、香港マイル、香港スプリント、香港ヴァーズ）、チャンピオンズデー（QE2世C、チャンピオンズマイル、チェアマンズスプリントプライズ）、香港三冠（スチュワーズC、香港ゴールドC、チャンピオンズ&チャターC）等を含む。
  - **データアーキテクチャ拡張 [完了]**:
    - `country_code`: 国コード `"HK"` をスキーマに完全適用。
    - `organization`: `"hkjc"` を新設。
    - タイムゾーン・発走時刻変換: 香港現地時間（香港時間 HKT: UTC+8、通年固定・夏時間なし）から正確に UTC ISO 8601 文字列（`start_time`）へ変換。
    - `LocalizedText`: 原語表記（繁体字中国語 `zh?: string`）を許容。
    - `AgeConstraint`: 4歳限定戦に対応する `'4yo'` を新設。
  - **日英中対応マスタ (`src/data/hk_race_master.json`) [完了]**:
    - レース名: 英語正式名（`Hong Kong Cup`, `Stewards' Cup`, `Hong Kong Derby` 等）$\leftrightarrow$ 繁体字中文名（`香港盃`, `董事盃`, `香港打吡大賽` 等）$\leftrightarrow$ 日本語通称（`香港カップ`, `スチュワーズカップ`, `香港ダービー` 等）のマッピング。
    - 競馬場名: 日英中対応（沙田 / Sha Tin / シャティン、跑馬地 / Happy Valley / ハッピーバレー）。
    - 抽出・マージモジュール（`scripts/lib/hk-races.ts`）の実装とビルドパイプライン（`scripts/parse-races.ts`）への統合（全1196レースへマージ）。
  - **UI/UX 拡張**:
     - 主催者フィルターへの「香港 (HKJC)」の追加（`All` / `JRA` / `NAR` / `France` / `UK` / `USA` / `Hong Kong`）。
     - 競馬場セレクトへの「香港 (HK)」グループ新設。
     - タイムラインビュー・カレンダービュー・詳細ダイアログでの国コード「HK」バッジ表示。
     - 免責事項ダイアログ（`DisclaimerDialog`）への Hong Kong Jockey Club (HKJC / 香港賽馬會) 出典・非公式性・知的財産権の明記。
  - **確定発走時刻自動更新 & 過去実績補完**:
...
- **海外主要レースのさらなる拡張**:
  - オーストラリア（Racing Australia）、UAE/ドバイ（ERA）等の重賞データ統合。
...
海外競馬の統合に伴い、レースの開催国を即座に識別できるよう、タイムライン、カレンダー、および詳細ダイアログに **ISO 2文字国コード（例: `FR`, `GB`, `US`, `HK`, `JP`）** を基盤とした軽量バッジを配置する。

- **仕様原則**:
  - 今後導入される海外競馬（UAE: `AE`、豪州: `AU` 等）を含め、国旗絵文字ではなくプラットフォーム非依存で一貫した可読性を保つため **2文字の国コードテキスト表記（`FR`, `GB`, `US`, `HK`）** で統一。
  - セマンティックトークン: フランス競馬向けには洗練されたトリコロール・フレンチブルー（`bg-sky-700` / `text-white`、コントラスト比 4.5:1 以上）、イギリス競馬向けには英国レーシングを象徴するディープネイビー（`bg-slate-800` / `text-white`、コントラスト比 4.5:1 以上）、アメリカ競馬向けには米国スターズ＆ストライプスを象徴するクラシックネイビー（`bg-blue-900` / `text-white`、コントラスト比 4.5:1 以上）、香港競馬向けにはバウヒニアと熱気あふれる香港競馬を象徴するオリエンタルレッド/クリムゾン（`bg-red-700` / `text-white`、コントラスト比 4.5:1 以上）を適用。
  - 日本国内レース（`JP`）については、過度な視覚的ノイズを抑制するためデフォルトではJRA/NARバッジを優先表示し、海外レース（`FR`, `GB`, `US`, `HK`）において明確な国識別バッジとして強調表示する。
...
- **香港重賞レース格付け・条件ソース**: IFHA / ICSC パートI リスト (Hong Kong)（参照元: [IFHA Resources](https://www.ifhaonline.org/Default.asp?section=Resources&area=8)）
- **香港競馬開催日程・出馬表ソース**: [The Hong Kong Jockey Club (HKJC / 香港賽馬會)](https://racing.hkjc.com/)
...
- **年齢制限 (`age_constraint`)**:
  ...
  - `/4歳|4yo/i` $\rightarrow$ `4yo`, ラベル: `{ ja: "4歳", en: "4yo" }`（※香港4歳クラシック等）
...
- **香港重賞マスター (`src/data/hk_race_master.json`)**:
  - **レース名辞書**: 英語正式名（`Hong Kong Cup`）、繁体字中文名（`香港盃`）、日本語通称名（`香港カップ`）をマッピング。
  - **競馬場辞書**: 沙田（Sha Tin / シャティン）、跑馬地（Happy Valley / ハッピーバレー）の日英中対訳。
  - **国コード**: `"HK"` を付与。
  - **タイムゾーン & 発走時刻変換規則**:
    - 香港現地時刻（香港時間 HKT: UTC+8 固定、通年夏時間なし）から厳密に UTC ISO 8601 文字列を算出（`HKT - 8時間 = UTC`）。
    - 昼間開催（沙田など）は主に現地 13:00〜18:00（JST 14:00〜19:00 / UTC 05:00〜10:00）、ナイター開催（跑馬地など）は現地 18:45〜23:00（JST 19:45〜24:00 / UTC 10:45〜15:00）で実施。マスタには各重賞の標準推定発走時刻を設定。
...
`races.json` は構築初期より多言語オブジェクト形式（`{ ja, en, fr?, zh? }`）で設計されており、JRA・NAR・ばんえい競馬・海外（フランス・イギリス・アメリカ・香港）重賞を統一スキーマで統合管理する。
...
- 書式: `{YYYY}-{org}-{grade_code}-{index}`
  ...
  - 香港G1例: `2026-hk-g1-01`（香港カップ、スチュワーズC等）
  - 香港G2例: `2026-hk-g2-01`（ジョッキークラブ各競走等）
  - 香港G3例: `2026-hk-g3-01`（ナショナルデイC等）
...
33. **Step 33 (v1.28.0): 海外競馬第3弾・アメリカ競馬（US / Equibase）の統合 (Issue #101, #102, #103, #104)**
    ...
    - **Phase 5: アメリカ競馬マスタの日付計算不具合（月末跨ぎ漏れによる実在しない日付）解消 & バリデーション強化 (Issue #106) [完了]**
      - 不具合の根本原因:
        - `src/data/us_race_master.json` 生成時の日付パース処理において、前年開催曜日（土曜）を2026年の同一曜日に合わせる補正計算（`targetDay += diff`）を行った際、月末日（30日/31日）を超過した場合に翌月へ繰り越す処理（月跨ぎ処理）が欠落。
        - これにより、全408レース中19レースにおいて `2026-08-36`, `2026-09-33`, `2026-11-35`, `2026-12-33` という不正な日付が出力されていた。
        - タイムラインビューでは JS の `new Date(year, month - 1, day)` がオーバーフローして有効扱いされる一方、フォーマット表示時にハイフン分割した数値を直接埋め込むため「2026年9月33日(土)」と描画され、カレンダービューでは実在する暦日（1日〜30日等）を走査するため該当レースが非表示となっていた。
      - 恒久対策の実施:
        - `src/data/us_race_master.json` の該当19レースの日付を正しい暦日（Flower Bowl S. `2026-09-05`、Goodwood S. 等の秋季主要重賞 `2026-10-03`、Hollywood Derby 等の感謝祭翌週重賞 `2026-12-05`、Mr. Prospector S. `2026-12-26`）および対応するタイムゾーンの `start_time` へ更新。
        - レース名に紛れ込んでいた Wiki 構文ゴミ（`Pebbles Stakes`）をクリーンアップ。
        - `scripts/parse-races.ts` を再実行し、`public/data/races.json` の全1,161レースについて日付整合性を同期更新。
      - 再発防止策（単体テスト拡充）:
        - `tests/unit/usRaces.test.ts` および `tests/unit/racesData.test.ts` に、全レースの `date` が実在する暦日（カレンダー上の日付）であること（`Date.UTC` 換算で年月日の数値が一致すること）およびレース名に不正なHTML・マークアップ文字が含まれないことを検証するアサーションを追加。全43スイート・392テスト完全合格。
    - **Phase 6: 開催国・競馬場増加に伴うフィルタービューの画面占有解消と主催者連動UI最適化 (Issue #107) [完了]**
      - 課題・背景: 開催国（フランス・イギリス・アメリカ等）の追加に伴い登録競馬場が73場に達し、スマートフォンで競馬場フィルターを展開した際にボタン群が画面全体を占有してレース一覧が隠れてしまう問題、および選択中の主催者に関係のない他国の競馬場や特殊グレード（Jpn、南関S、地方重賞等）が無条件で常時表示される問題を解消。
      - 主催者（開催国）選択と競馬場グループの動的連動:
        - `filters.organizations` が選択されている場合、該当する主催者に属する競馬場グループのみを動的に絞り込んで表示（例: アメリカ選択時は米国の16場のみ、フランス選択時は仏国の16場のみ、JRA+NAR選択時は日本国内の4グループのみ）。
        - 主催者未選択（全主催者表示）時でも、競馬場パネル上部に地域クイックセレクター（`すべて` / `🇯🇵 日本` / `🇪🇺 欧州` / `🇺🇸 米国`）を提供し、地域ごとにワンタップで絞り込み可能に。
      - 主催者選択に応じたグレード・馬場種別選択肢の最適化:
        - 海外主催者（フランス・イギリス・アメリカ）のみ選択時は、日本特有の障害重賞（`J.G1〜J.G3`）、ダートグレード（`Jpn1〜Jpn3`）、南関重賞（`S1〜S3`）、地方重賞（`local_grade`）および一括トグルボタンを非表示化し、`G1〜G3` と `AW` 馬場を表示。
        - JRAのみ選択時は `G1〜G3`, `J.G1〜J.G3` のみ表示、NARのみ選択時は `G1〜G3`, `Jpn1〜Jpn3`, `S1〜S3`, `地方重賞`, 一括トグル, `ばんえい` を表示。
      - フィルターパネルの高さ制限 & 内部スクロールによる画面突き抜け防止:
        - 競馬場選択パネル（`course-filter-panel`）のグループ一覧に `max-h-60 sm:max-h-80 overflow-y-auto pr-1` を付与し、全73場表示時でも約240pxでスクロール可能に制御。
        - 詳細フィルターパネル全体（`#detailed-filters-panel`）に `max-h-[75vh] overflow-y-auto` を付与し、スマホ画面でのレース一覧視認性を担保。
      - 単体テストの拡充: `FilterBar.test.tsx` に主催者連動（競馬場グループ、グレード、馬場種別）、地域タブ、高さ制限・内部スクロールクラスの検証テストを追加（全43スイート・399テスト完全合格）。
