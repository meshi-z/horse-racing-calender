...
v1.16.0 で導入されたUI多言語（日/英）対応（i18n）基盤の上に、v1.17.0 では **NAR（地方競馬）全重賞および世界唯一の直線そりレースである「ばんえい競馬」のデータパイプラインとUI完全対応** を実現する。
...
- **NAR（地方競馬）全重賞 & ばんえい競馬対応 [開発完了 / Current]**:
  ...
  - **当週NAR確定発走時刻自動取得**: `RaceTimeFetcher` プロバイダーアーキテクチャへの `NarRaceTimeFetcher` 追加とダートグレード競走日程からの確定時刻自動更新。
