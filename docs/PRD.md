...
| **バージョン** | v1.20.1 (NAR過去重賞データの発走時刻バックフィルおよびバックフィルスクリプトの整備) |
...
v1.16.0 の多言語（日/英）対応、v1.17.0 のNAR全重賞・ばんえい競馬対応、v1.18.0 のアコーディオンフィルター、v1.18.1 のNAR英語名整理、v1.18.2 のヘッダー英語・検索例・SEO再整理、v1.19.0 のフランス平地重賞（France-Galop / IFHA Part I）統合、v1.20.0 のNAR確定発走予定時刻自動更新パイプライン拡充を経て、**v1.20.1 ではNAR公式出馬表（RaceList）アーカイブから本日以前に開催されたNAR全199重賞の発走実績時刻を一括スクレイピングし、確定時刻（is_time_confirmed: true）へのバックフィルを完全完了** した。
...
27. **Step 27 (Current / v1.20.0 〜 v1.20.1): NAR確定発走予定時刻の自動更新パイプライン拡充および過去重賞バックフィル [完了]**
...
28. **Step 28 (Next): JRA過去重賞実績発走時刻マスタ整備 & 海外主要レース拡張**
    - JRA過去重賞（98レース）の実績発走時刻マスタ整備および確定更新。
