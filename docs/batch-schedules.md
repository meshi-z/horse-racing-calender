...
| **年間データ一括ビルド** | `scripts/parse-races.ts` | 手動実行 (ローカル) | 年間更新時、開催日程・マスタ辞書更新時 | オンデマンド | JRA公式ICS/HTML、NARスケジュールHTML、フランス競馬マスタ、イギリス競馬マスタ、アメリカ競馬マスタ、香港競馬マスタから全レースデータを統合マージして再生成 | 入力: 各マスタ/公式データ<br>出力: `public/data/races.json` |
...
#### 概要・設計根拠
- **対象レース**: JRA当週開催レース（木〜翌月曜）、NAR直近7日間の全重賞（ダートグレード競走、南関重賞、各地区地方重賞、ばんえい重賞）、フランス競馬（France Galop 直近7日間）、イギリス競馬（BHA 直近7日間）、アメリカ競馬（Equibase 直近7日間）、および香港競馬（HKJC 直近7日間）。
- **実行スケジュールの根拠**:
  ...
  - **毎日夜 (21:30 JST)**: 欧州競馬（フランス・イギリス重賞）、アメリカ競馬、および香港競馬（跑馬地ナイター開催等）の当日開催直前・確定確認。
...
# 特定の主催者のみを対象に実行（'jra', 'nar', 'france_galop', 'bha', 'equibase', 'hkjc'）
...
npm run data:update-times:hk   # または npm run data:update-times -- --org=hkjc
```
