...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **確定発走時刻自動更新** | `.github/workflows/update-race-times.yml`<br>`scripts/update-race-times.ts` | GitHub Actions (cron)<br>手動 (`workflow_dispatch`) | **木曜**: 16:30, 17:00, 18:00<br>**金曜**: 10:30, 11:00, 12:00<br>**土曜**: 07:30, 17:30<br>**日曜**: 07:30 | `30 7 * * 4`<br>`0 8 * * 4`<br>`0 9 * * 4`<br>`30 1 * * 5`<br>`0 2 * * 5`<br>`0 3 * * 5`<br>`30 22 * * 5`<br>`30 8 * * 6`<br>`30 22 * * 6` | JRA公式出馬表およびNARダートグレード出馬表から直近レースの確定発走時刻・代替開催日を取得し更新 | 入力: 公式サイトHTML<br>出力: `public/data/races.json` |
...
| **年間データ一括ビルド** | `scripts/parse-races.ts` | 手動実行 (ローカル) | 年間更新時、開催日程・マスタ辞書更新時 | オンデマンド | JRA公式ICS/HTML、NARスケジュールHTML、フランス競馬マスタから全レースデータを統合マージして再生成 | 入力: 各マスタ/公式データ<br>出力: `public/data/races.json` |
...
#### 概要・設計根拠
- **対象レース**: JRA当週開催レース（木〜翌月曜）および NAR直近7日間のダートグレード競走。
- **実行スケジュールの根拠**:
  - **木曜日 (16:30, 17:00, 18:00 JST)**: JRAの出馬表発表（通常16:00頃）に合わせ、段階的に確定時刻をフェッチ。
  - **金曜日 (10:30, 11:00, 12:00 JST)**: JRAの確定枠順・発走時刻発表（通常10:00頃）およびNAR当週レースの出走確定を網羅。
  - **土曜日 (07:30, 17:30 JST)**: 土曜当日朝の最終確認（悪天候による順延・代替開催等）および日曜前日夕方の確認。
  - **日曜日 (07:30 JST)**: 日曜当日朝の最終確認。

#### ローカル実行コマンド
```bash
# 通常実行（直近ウィンドウ内の未確定レースのみ取得・更新）
