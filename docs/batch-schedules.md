...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **確定発走時刻自動更新** | `.github/workflows/update-race-times.yml`<br>`scripts/update-race-times.ts` | GitHub Actions (cron)<br>手動 (`workflow_dispatch`) | **毎日朝**: 07:30<br>**平日昼**: 12:30<br>**平日〜土曜夕**: 17:30<br>**木曜**: 16:30, 17:00, 18:00<br>**金曜**: 10:30, 11:00, 12:00 | `30 22 * * *`<br>`30 3 * * 1-5`<br>`30 8 * * 1-6`<br>`30 7 * * 4`<br>`0 8 * * 4`<br>`0 9 * * 4`<br>`30 1 * * 5`<br>`0 2 * * 5`<br>`0 3 * * 5` | JRA公式出馬表およびNAR公式出馬表（ダートグレード年間日程＋各競馬場RaceList）から直近レースの確定発走時刻・代替開催日を取得し更新 | 入力: 公式サイトHTML<br>出力: `public/data/races.json` |
...
#### 概要・設計根拠
- **対象レース**: JRA当週開催レース（木〜翌月曜）および NAR直近7日間の全重賞（ダートグレード競走、南関重賞、各地区地方重賞、ばんえい重賞）。
- **実行スケジュールの根拠**:
