...
#### 概要・設計根拠
- **対象レース**: JRA当週開催レース（木〜翌月曜）、NAR直近7日間の全重賞（ダートグレード競走、南関重賞、各地区地方重賞、ばんえい重賞）、フランス競馬（France Galop 直近7日間）、イギリス競馬（BHA 直近7日間）、アメリカ競馬（Equibase 直近7日間）、香港競馬（HKJC 直近7日間）、およびアイルランド競馬（HRI 直近7日間）。
- **実行スケジュールの根拠**:
  ...
  - **毎日夜 (21:30 JST)**: 欧州競馬（フランス・イギリス・アイルランド重賞）、アメリカ競馬、および香港競馬（跑馬地ナイター開催等）の当日開催直前・確定確認。
...
# 特定の主催者のみを対象に実行（'jra', 'nar', 'france_galop', 'bha', 'equibase', 'hkjc', 'hri'）
...
npm run data:update-times:ie   # または npm run data:update-times -- --org=hri
```
